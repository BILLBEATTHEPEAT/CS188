...
def enhancedFeatureExtractorDigit(datum):
    ...
    '''
    We calculate the number of blank in the picture. Since we can know that the blanks
    are very similar in 1, 7 and the like and different among 1, 8, 0, etc, we can use
    this as a feature.
    '''
    ##
    """
    ...
    mask = [[0]*len(pixels[0]) for _ in range(len(pixels))]

    for i in range(len(pixels)):
        for j in range(len(pixels[0])):
            if mask[i][j] == 0 and pixels[i][j] == 0:
                blank += 1
                dfs(pixels, mask, i, j)
    if blank > 1:
        features["blank"] = 1
...
def dfs(map, mask, i, j):
    mask[i][j] = 1
    if 0 <= i-1:
        if map[i-1][j] == 0 and mask[i-1][j] == 0:
            dfs(map, mask, i-1, j)
    if 0 <= j-1:
        if map[i][j-1] == 0 and mask[i][j-1] == 0:
            dfs(map, mask, i, j-1)
    if i+1 < len(map):
        if map[i+1][j] == 0 and mask[i+1][j] == 0:
            dfs(map, mask, i+1, j)
...
def enhancedPacmanFeatures(state, action):
    ...
    ghostDist = [util.manhattanDistance(position, ghost) for ghost in ghosts]
    foodDist = [util.manhattanDistance(position, food) for food in foods]
    capsuleDist = [util.manhattanDistance(position, capsule) for capsule in capsules]

    # features['score'] = score * 0.7
    features['closestGhost'] = min(ghostDist)
    features['closestFood'] = 1.0/min(foodDist) if foodDist else 1.0
    features['closestCapsule'] = min(capsuleDist) if capsuleDist else 0
